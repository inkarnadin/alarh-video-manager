...
@CompileStatic
class MonitorProcessBuilder implements SystemProcessBuilder {

  @Override
  ProcessBuilder build(Target target, int counterValue) {
    String url = WebUtils.buildRTSPLink(target)
    String path = FileUtils.getOrCreateTargetDirectory("monitoring", "screen")
    ...
    return new ProcessBuilder()
      ...
      .command("ffmpeg",
        ...
        "-vframes", "1",
        "-q:v", "2",
        "$path/$target.host-$target.name-$counterValue" + ".jpg")
  }
